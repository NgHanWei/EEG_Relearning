    ...
    def run(self):
        ...
        for subj in range(1,55):
            ...
            for i in range(1,5):
                ...
                full_loss = recon_loss(reconstruction,X_test[(i-1)*100:i*100])

                loss_list.append(full_loss.item())
                print(full_loss)

                data_list.append(X_test[(i-1)*100:i*100])
                label_list.append(y_test[(i-1)*100:i*100])
        ...
        def ceil(a, b):
            return -1 * (-a // b)
        print(len(sort_list))
        for loss in sort_list:
            print(loss)
            sub_index = loss_list.index(loss) + 1
       

            print("SUBJECT: " + str(ceil(sub_index,4)))
            index_list.append(ceil(sub_index,4))
            phase = sub_index - (sub_index//4)*4
            if phase == 0:
                phase = 4
        ...
        if targ_trial == -1:
            ...
            save_string = './subj_phase_lists/subj_' + str(self.subj) + '_list.npy'
            save_string_2 = './subj_phase_lists/subj_phase_' + str(self.subj) + '_list.npy'
        else:
            if os.path.exists("./trial_phase_lists/") == False:
                os.makedirs("./trial_phase_lists/")
            save_string = './trial_phase_lists/test_' + str(self.subj) + '_list.npy'
            save_string_2 = './trial_phase_lists/test_phase_' + str(self.subj) + '_list.npy'
        with open(save_string, 'wb') as f:
            np.save(f, np.array(index_list[:172]))
        with open(save_string_2, 'wb') as f:
            np.save(f, np.array(phase_list[:172]))